...
## 2026-07-13 (15차) — 미입력 카드 수정에서 이름·연락처 입력 + 상세 숨김 토글 (마이그레이션 없음, 배포 완료)

### 사용자 요구 → 조치
1. **미입력(identityPending) 카드가 수정 화면에서 이름·연락처 잠김 문제**: §7.5 불변 잠금은 "최초 등록 후"가 전제인데 수정 폼이 무조건 잠갔음 → `ProfileForm`에 `identityPending` prop 신설 (수정 페이지에서 전달). 미입력 카드 수정 시:
   - 이름·연락처 입력란 활성화 + 안내 배너("입력하면 공개, 이후 변경 불가, 안 채워도 다른 항목 수정 가능") + 당사자 동의 체크박스
   - 이름·연락처는 선택 입력 — 채우면 둘 다+동의 필수 검증, 저장 시 **fillIdentity를 먼저 호출**(공개 전환·클레임 연동·알림 기존 로직 재사용) 후 update. 이전 시도에서 이미 입력된 경우(400 "이미 정보가 입력된")는 무시하고 update 계속 (재시도 안전)
   - fillIdentity 권한에 관리자 추가 (DB role 확인)
   - 스펙 §7.5에 미입력 카드 예외 명문화
2. **숨김(비공개) 진입점 추가**: HideToggle이 MY>내 프로필에만 있어 지인 카드는 숨길 방법이 없었음 → 프로필 상세 하단에 HideToggle 배치 (편집 가능자+관리자, MATCHED 제외). 숨김 상태면 "🙈 숨김 상태" 안내 문구 동반. 스펙 §7.11에 추가

### 메모
- 마이그레이션 없음. `npm run build` 통과. fillIdentity→update 순차 호출 사이 실패 시 부분 저장 가능하지만 재시도로 수렴 (위 400 무시 처리)

---

## 2026-07-13 (14차) — 카드 삭제 + 관리자 전체 편집·삭제·사진 열람 (마이그레이션 없음, 배포 완료)

### 사용자 요구 → 조치
1. **카드 삭제 기능 신설** (스펙 §7.11): 잘못 등록·지인이 타처에서 커플 된 경우 정리용
   - `DELETE /api/profiles/[id]` — 완전 삭제(탈퇴의 익명화와 달리 행 제거). 트랜잭션으로 신고·감지 플래그·매칭 제안·호감(Like)·사진 열람권(이 카드의 호감으로 발급된 양방향 포함)·열람 기록·관련 알림(payload.profileId 매칭)·사진 삭제 후 프로필 행 삭제. Storage 파일은 DB 정리 후 삭제
   - 권한: 편집 가능자(canEditProfile) + 관리자. **성사(Match) 이력 있으면 일반 회원은 400** (숨기기 안내) — 관리자는 강제 삭제 가능, 이때 Match 행 삭제 + 상대 프로필 MATCHED→ACTIVE 복귀
   - UI: 프로필 상세 우상단 "🗑 삭제" (`DeleteProfileButton.tsx`, 인라인 확인 팝오버 — 되돌릴 수 없음 고지). 삭제 후 본인 카드는 /me, 지인 카드는 /deck으로 이동
2. **관리자 전체 카드 편집·삭제** (스펙 §12.4 확대): `profileAccess.ts`에 `canManageProfile()` 신설 (canEditProfile ∪ ADMIN, DB role 재확인). 프로필 PATCH(수정·숨김·공유 등)/사진 POST·PATCH·DELETE 전부 교체. 상세 페이지 canEdit·수정 페이지 가드에 JWT role 기반 isAdmin 추가
3. **관리자 전체 사진 열람** (스펙 §7.2 v1.6): `photoGate.ts` canViewPhotos에 ADMIN 통과 추가 (DB role 확인) — 상세 페이지·GET API 자동 적용

### 메모
- 마이그레이션 없음 (스키마 변경 없음). `npm run build` 통과
- 검증은 빌드+코드 경로 확인까지 — 삭제 E2E는 라이브 DB 공유라 실데이터 미생성 원칙으로 미진행. 실사용 첫 삭제 시 확인 권장
- 알림 정리는 payload.profileId 기준만 — likeId만 담긴 알림은 남을 수 있음 (클릭 시 해당 화면에서 자연 소멸, 소규모 운영 수용)
